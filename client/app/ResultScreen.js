...
const ResultScreen = () => {
    ...
    const [recycleF, setRecycleF] = useState(false)
    ...
    const getBarcode = async () => {
        try {
            const response = await axios.get('http://localhost:8000/barcode', {
                params: {barcode}
            })
    ...
    const handleInsert = async () => {
        try {
            const response = await axios.post('http://localhost:8000/additem', {title, barcode, image, recycle})
